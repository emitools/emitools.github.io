...
function calculateEMI() {
    ...
    for (let month = 1; month <= N && balance > 0; month++) {
        ...
        // Aggregating for yearly bar chart
...
function updateUI(emi, totalInterest, totalPayment, schedule, yearlyData, principal) {
    ...
    schedule.forEach(row => {
        ...
        tr.innerHTML = `
            ...
            <td style="color:var(--secondary-color)">${row.prepayment > 0 ? formatCurrency(row.prepayment) : '-'}</td>
